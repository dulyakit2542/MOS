...
?>
	...
	<body>
		<section class="pt-7 pt-md-5">
			...
			<div class="row justify-content-center">
				...
				?>
					<input type="text" style="display:none;" id="ce" value="<?php echo $ce; ?>">
					<?php
					if ($ce > 0) { ?>
						<?php
								...
								<div class="card rounded-0 card-hover-overlay">
									...
									<div class="card-footer bg-transparent">
										<h6 class="h6" style="justify-content: center; align-items: center;">
											<input type="text" style="display:none;" id="exam_date" value="<?php echo $date; ?>">
											วันที่สอบ : <strong><span id="get_exam_date"></span></strong>
										</h6>
										<h6 class="h6" style="justify-content: center; align-items: center;">
										...
										if ($p->exam_member_status == 'wait') {
											echo '<h5 class="h6">สถานะ : <strong style="color:#006EFF;">รอเจ้าหน้าที่อนุมัติ<h5></strong>';
										} elseif ($p->exam_member_status == 'waiting_exam') {
											echo '<h5 class="h6">สถานะ : <strong style="color:#006EFF;">สามารถเข้าสอบได้<h5></strong>';
											// echo '<h5 class="h6">ชุดข้อสอบที่ได้ : <strong style="color:#006EFF;">'.$p->exam_archive_name.'<h5></strong>';
									...
									<div class="card2">
										...
										<form action="" method="POST">
											<div class="card-body card-body-cascade text-center">
												...
												?> <input type="text" style="display:none;" id="train_date" value="<?php echo $date; ?>">
													<h5 class="h6">สถานะ : <strong style="color:green;">ผ่านการอบรม<h5></strong>
														<h5 class="h6">อนุมัติเมื่อ : <strong><span id="get_train_date"></span></h5></strong><br>
													<?php
												} else if ($p->status_train == 'notpass') {
													?>
														<h5 class="h6">สถานะ : <strong style="color:red;">ไม่ผ่านการอบรม หากต้องการอบรมอีกครั้งกรุณากดยกเลิกลงทะเบียนแล้วลงทะเบียนใหม่<h5></strong>
															<h5 class="h6">อนุมัติเมื่อ : <strong><span id="get_train_date"></span></h5></strong><br>
														<?php
													} else if ($p->wait_for_test == 'wait') {
														?>
															<h5 class="h6">ยืนยันการอบรมเมื่อ : <strong style="color:#006EFF;"><?php echo $p->wait_date_for_test; ?><h5></strong>
																...
																?>
																	<input type="text" style="display:none;" id="regis_train_date" value="<?php echo $date; ?>">
																	<h5 class="h6">สถานะ : <strong>กำลังอบรม<h5></strong>
																		<h5 class="h6">ลงทะเบียนเมื่อ : <strong><?php echo $date; ?></span>
																		<h5></strong><br>
																		<?php
																	}
																		?>
			...
			<div class="col-lg-12">
				<div class="md-form form-sm mb-0"><br>
					...
					<h6 style="color:red">
						- ผู้ใช้มีสิทธิ์เข้าสอบ 3 ครั้งเท่านั้น หากครั้วที่ 3 สอบไม่ผ่านจะต้องรอลงทะเบียนใหม่ครั้งถัดไป
					</h6>
				</div>
			</div>
		</section><br>
	...
	<script>
		...
		if (ce > 0) {
			var date1 = "";
			var date1 = document.getElementById("exam_date").value;
			document.getElementById("get_exam_date").innerHTML = dateThaiJs(date1);
		}
		var status_train = ''
		var status_train = document.getElementById("status_train");
		if (status_train = 'pass') {
			var date = "";
			var date = document.getElementById("train_date").value;
			document.getElementById("get_train_date").innerHTML = dateThaiJs(date);
		}
		if (status_train = 'notpass') {
			var date = "";
			var date = document.getElementById("train_date").value;
			document.getElementById("get_train_date").innerHTML = dateThaiJs(date);
		}
